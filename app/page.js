...
const API_KEY = 'TU_API_KEY_AQUI'; // <--- PON TU CLAVE DE RAPIDAPI AQUÍ
...
export default function Home() {
  ...
  // ======================= ESTADOS MAESTROS =======================
  const [vistaActiva, setVistaActiva] = useState('PORTADA'); // PORTADA | LIGA1_2026 | CAMPEONES | HISTORICO
  const [subVista, setSubVista] = useState('FIXTURE'); // FIXTURE | EQUIPOS
  ...
  // ======================= PORTADA / CALENDARIO / VIVO =======================
  const today = useMemo(() => new Date('2026-03-30T12:00:00-05:00'), []); // Fijado a tu fecha de contexto
  ...
  // Generador básico de días del mes para el calendario popup
  ...
  useEffect(() => {
    const fetchPartidos = async () => {
      ...
      try {
        ...
        if (data.status === 'success' && data.response?.matches) {
          const partidosPeru = data.response.matches.filter(p => String(p.leagueId) === String(LIGA_ID));
          setPartidosPortadaListado(partidosPeru.map(p => ({
        ...
        // Fallback a Mock Data 2026
        const targetStr = `${year}-${month}-${day}`;
        const locales = partidos2026Base.filter(p => p[5] && p[5].startsWith(targetStr)).map(p => ({
    ...
    if (vistaActiva === 'PORTADA') fetchPartidos();
  }, [selectedDate, vistaActiva]);

  const tituloFechaHeader = useMemo(() => {
    ...
    if (isToday) return "PARTIDOS DE HOY";
    if (isYesterday) return "PARTIDOS DE AYER";
    if (isTomorrow) return "PARTIDOS DE MAÑANA";
    return `${selectedDate.getDate()} ${selectedDate.toLocaleString('es-ES', { month: 'short' }).toUpperCase()} ${selectedDate.getFullYear()}`;
  }, [selectedDate, today]);

  // ======================= LÓGICA HISTÓRICA / TABLAS =======================
  ...
  const ganadorMesa = (p) => {
    ...
    if (p.GL !== null && p.GV !== null && p.Torneo !== 'Final') return p.GL > p.GV ? p.Local : (p.GL < p.GV ? p.Visitante : null);
    return null;
  };
  ...
  const listaPartidos = useMemo(() => {
    ...
    if (temporada === '2023') {
      ...
      const orden = { 3:1, 4:2, 5:3, 6:4, 7:5, 8:6, 9:7, 1:8, 10:9, 11:10, 12:11, 13:12, 14:13, 15:14, 2:15, 16:16, 17:17, 18:18, 19:19 };
      return raw2023.map(p => {
        let jo = p[0]; let t = p[5] || 'Apertura'; let fg = (t === 'Apertura' && orden[jo]) ? orden[jo] : jo;
        return { Jornada_Oficial: jo, Fecha_Global: fg, Torneo: t, Local: normalizarEquipoListado(p[1]), Visitante: normalizarEquipoListado(p[2]), GL: p[3], GV: p[4] };
      });
    }
    if (temporada === '2013') {
  ...
  return (
    <div className="flex h-screen bg-[#0a1c14] font-sans overflow-hidden selection:bg-[#8cc63f] selection:text-black">
      ...
      {/* ======================= SIDEBAR (IZQUIERDA) ======================= */}
      <aside className="w-[240px] bg-[#112a1f] text-white flex-shrink-0 h-screen sticky top-0 overflow-y-auto hidden md:flex flex-col shadow-xl z-20 border-r border-[#1f4a36] custom-scrollbar">
        
        {/* Header del Sidebar */}
        <div 
          onClick={() => { setVistaActiva('PORTADA'); setSelectedDate(today); }}
          className="p-4 flex items-center gap-3 border-b border-[#1f4a36] cursor-pointer hover:bg-[#1a3d2c] transition-colors"
        >
        ...
        <div className="flex-1 py-2 flex flex-col">
          ...
          {/* SECCIÓN PERÚ */}
          <div className="px-3">
            <button onClick={() => setPeruOpen(!peruOpen)} className="w-full flex items-center justify-between text-gray-300 hover:text-white py-2 font-bold text-[11px] uppercase tracking-wider outline-none">
              <div className="flex items-center gap-2">
                <img src="https://tmssl.akamaized.net//images/holding/head/pe.png" className="w-[14px]" /> PERÚ
              </div>
              <span className="text-[9px]">{peruOpen ? '▲' : '▼'}</span>
            </button>
          </div>
          ...
          {peruOpen && (
            <div className="flex flex-col mt-1 mb-2">
              <button
                onClick={() => { setVistaActiva('LIGA1_2026'); setTemporada('2026'); setFecha(8); setSubVista('FIXTURE'); }}
                className={`w-full flex items-center gap-3 px-8 py-2 text-[12px] font-bold transition-all outline-none ${vistaActiva === 'LIGA1_2026' ? 'bg-[#1f4a36] border-l-[3px] border-[#8cc63f] text-white' : 'text-gray-400 hover:bg-[#1a3d2c] hover:text-white border-l-[3px] border-transparent'}`}
              >
                Liga 1 Te Apuesto
              </button>

              <button onClick={() => setCampeonesOpen(!campeonesOpen)} className="w-full flex items-center justify-between text-gray-400 hover:text-white px-8 py-2 text-[12px] font-bold transition-all outline-none mt-1">
                <span>Campeones</span>
                <span className="text-[9px]">{campeonesOpen ? '▲' : '▼'}</span>
              </button>

              {campeonesOpen && (
                <div className="flex flex-col bg-[#0a1c14] py-1 border-y border-[#1f4a36] mt-1">
                  <button onClick={() => { setVistaActiva('HISTORICO'); setTemporada('2023'); setFecha(38); setSubVista('FIXTURE'); }} className={`w-full text-left pl-10 py-1.5 text-[11px] font-semibold outline-none transition-colors ${vistaActiva === 'HISTORICO' && temporada === '2023' ? 'text-[#8cc63f]' : 'text-gray-400 hover:text-white'}`}>2023 - Universitario</button>
                  <button onClick={() => { setVistaActiva('HISTORICO'); setTemporada('2018'); setFecha(44); setSubVista('FIXTURE'); }} className={`w-full text-left pl-10 py-1.5 text-[11px] font-semibold outline-none transition-colors ${vistaActiva === 'HISTORICO' && temporada === '2018' ? 'text-[#8cc63f]' : 'text-gray-400 hover:text-white'}`}>2018 - Sporting Cristal</button>
                  <button onClick={() => { setVistaActiva('HISTORICO'); setTemporada('2013'); setFecha(44); setSubVista('FIXTURE'); }} className={`w-full text-left pl-10 py-1.5 text-[11px] font-semibold outline-none transition-colors ${vistaActiva === 'HISTORICO' && temporada === '2013' ? 'text-[#8cc63f]' : 'text-gray-400 hover:text-white'}`}>2013 - Universitario</button>
                </div>
              )}
            </div>
          )}

          {/* SECCIÓN INTERNACIONAL (Preparada para el futuro) */}
          <div className="px-3 mt-2 border-t border-[#1f4a36] pt-2">
            <button onClick={() => setInterOpen(!interOpen)} className="w-full flex items-center justify-between text-gray-300 hover:text-white py-2 font-bold text-[11px] uppercase tracking-wider outline-none">
              <div className="flex items-center gap-2">
                <span className="text-[14px]">🌎</span> INTERNACIONAL
              </div>
              <span className="text-[9px]">{interOpen ? '▲' : '▼'}</span>
            </button>
          </div>

          {interOpen && (
            <div className="flex flex-col mt-1">
              <button className="w-full flex items-center gap-3 px-8 py-2 text-[12px] font-bold text-gray-500 cursor-not-allowed outline-none border-l-[3px] border-transparent">
                Copa Libertadores
              </button>
              <button className="w-full flex items-center gap-3 px-8 py-2 text-[12px] font-bold text-gray-500 cursor-not-allowed outline-none border-l-[3px] border-transparent">
                Copa Sudamericana
      ...
      <div className="flex-1 flex flex-col h-screen overflow-y-auto w-full custom-scrollbar bg-[#0a1c14]">
        
        <div className="w-full max-w-[1000px] mx-auto p-4 md:p-6 pb-20">
          
          {/* ---------------- VISTA 1: PORTADA EN VIVO (PROMIEDOS STYLE) ---------------- */}
          {vistaActiva === 'PORTADA' && (
            <div>
              {/* Header Navegación Fechas con Calendario */}
              <div className="bg-[#112a1f] rounded-t-lg flex items-center justify-between p-3 text-white border-b border-[#1f4a36] relative">
                 <button onClick={() => { const prev = new Date(selectedDate); prev.setDate(prev.getDate() - 1); setSelectedDate(prev); }} className="hover:text-[#8cc63f] font-bold px-3 outline-none transition-colors text-lg">{'<'}</button>
                 
                 <div className="relative">
                   <button 
                     onClick={() => setIsCalendarOpen(!isCalendarOpen)} 
                     className="font-black text-[13px] tracking-widest uppercase hover:text-[#8cc63f] transition-colors outline-none flex items-center gap-2"
                   >
                     {tituloFechaHeader} <span className="text-[10px]">▼</span>
                   </button>

                   {/* Modal Calendario Simplificado */}
                   {isCalendarOpen && (
                     <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[260px] bg-[#112a1f] border border-[#1f4a36] rounded-lg shadow-2xl z-50 p-3">
                       <div className="text-center font-bold text-[#8cc63f] mb-3 text-sm border-b border-[#1f4a36] pb-2">
                         {selectedDate.toLocaleString('es-ES', { month: 'long', year: 'numeric' }).toUpperCase()}
                       </div>
                       <div className="grid grid-cols-7 gap-1 text-center">
                         {['LU','MA','MI','JU','VI','SA','DO'].map(d => <div key={d} className="text-[10px] font-bold text-gray-400 mb-1">{d}</div>)}
                         {diasDelMes.map(d => (
                           <button 
                             key={d} 
                             onClick={() => {
                               const newDate = new Date(selectedDate); newDate.setDate(d);
                               setSelectedDate(newDate); setIsCalendarOpen(false);
                             }}
                             className={`p-1.5 text-xs font-bold rounded-md hover:bg-[#8cc63f] hover:text-[#112a1f] transition-colors ${selectedDate.getDate() === d ? 'bg-[#8cc63f] text-[#112a1f]' : 'text-gray-200'}`}
                           >
                             {d}
                           </button>
                         ))}
                 ...
                 <button onClick={() => { const next = new Date(selectedDate); next.setDate(next.getDate() + 1); setSelectedDate(next); }} className="hover:text-[#8cc63f] font-bold px-3 outline-none transition-colors text-lg">{'>'}</button>
              </div>

              {/* Tabs Filtros */}
              <div className="bg-[#112a1f] px-4 flex items-center gap-6 border-b border-[#1f4a36]">
                 <button onClick={()=>setFiltroVivo('TODOS')} className={`py-3 text-[11px] font-bold outline-none uppercase transition-colors ${filtroVivo === 'TODOS' ? 'text-[#8cc63f] border-b-2 border-[#8cc63f]' : 'text-gray-400 hover:text-white'}`}>TODOS</button>
                 <button onClick={()=>setFiltroVivo('VIVO')} className={`py-3 text-[11px] font-bold outline-none uppercase transition-colors flex items-center gap-1 ${filtroVivo === 'VIVO' ? 'text-[#8cc63f] border-b-2 border-[#8cc63f]' : 'text-gray-400 hover:text-white'}`}>
                   VIVO <span className="bg-[#1f4a36] text-white px-1.5 py-0.5 rounded-full text-[9px]">{partidosPortadaListado.filter(p => p.EstadoApi !== 6 && p.EstadoApi !== 7 && p.GL !== null).length}</span>
                 </button>
              </div>

              {/* Bloque LIGA 1 */}
              <div className="bg-[#1f4a36] px-3 py-2 mt-4 flex items-center justify-between text-white text-[11px] font-bold rounded-t-md shadow-sm border border-[#1f4a36]">
              ...
              <div className="bg-[#112a1f] flex flex-col rounded-b-md overflow-hidden shadow-lg border border-[#1f4a36] border-t-0">
                  {cargandoApi ? (
                    <div className="text-center text-[12px] p-[30px] text-[#8cc63f] font-bold animate-pulse">Conectando con la cabina de transmisión... 🎙️⚽</div>
                  ) : partidosParaMostrarLive.length === 0 ? (
                    <div className="text-center text-[12px] p-[30px] text-gray-400 font-medium">No se encontraron partidos para la fecha seleccionada.</div>
                  ) : (
                    partidosParaMostrarLive.map((m, idx) => {
                       ...
                       return (
                         <div key={idx} className="flex border-t border-[#1f4a36] text-white hover:bg-[#1a3d2c] transition-colors cursor-pointer group">
                            {/* Columna Minuto/Estado */}
                            <div className="w-[60px] flex flex-col items-center justify-center border-r border-[#1f4a36] py-3 px-1">
                               <span className={`text-[11px] font-black ${enVivo ? 'text-[#d32f2f] animate-pulse' : 'text-gray-300 group-hover:text-white transition-colors'}`}>
                            ...
                            <div className="flex-1 flex flex-col justify-center p-3">
                                <div className="flex justify-center items-center gap-3 md:gap-6">
                                    <span className={`text-right w-[40%] text-[13px] font-bold truncate ${ganador === m.Local ? 'text-[#8cc63f]' : 'text-gray-200'}`}>{m.Local}</span>
                                    <img src={logos[m.Local]} className="w-6 h-6 object-contain drop-shadow-md"/>
                                    
                                    <span className="font-black text-[15px] bg-[#0a1c14] px-3 py-[3px] rounded border border-[#1f4a36] min-w-[55px] text-center tracking-widest text-[#8cc63f] shadow-inner">
                                      {m.GL !== null ? `${m.GL} - ${m.GV}` : ' - '}
                                    </span>
                                    
                                    <img src={logos[m.Visitante]} className="w-6 h-6 object-contain drop-shadow-md"/>
                                    <span className={`text-left w-[40%] text-[13px] font-bold truncate ${ganador === m.Visitante ? 'text-[#8cc63f]' : 'text-gray-200'}`}>{m.Visitante}</span>
                                </div>
                            </div>
                         </div>
                       );
          ...
          {/* ---------------- VISTA 2 y 3: TABLAS HISTÓRICAS / ACTUALES ---------------- */}
          {(vistaActiva === 'LIGA1_2026' || vistaActiva === 'HISTORICO') && (
            <div className="bg-[#e5eee9] rounded-xl p-4 md:p-6 shadow-2xl relative">
              
              {/* Header Interno de Sección */}
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 border-b border-[#d1e0d7] pb-4 gap-4">
                <h2 className="text-[20px] font-black uppercase text-[#112a1f] flex items-center gap-2 m-0">
                  <span className="bg-[#8cc63f] w-3 h-6 rounded-sm"></span>
                  LIGA 1 - TEMPORADA {temporada}
                </h2>
                
                {/* Mini-Tabs (Fixture vs Equipos) */}
                <div className="flex bg-white rounded-lg p-1 border border-[#d1e0d7] shadow-sm">
                  <button onClick={() => {setSubVista('FIXTURE'); setEquipoSeleccionado(null);}} className={`px-4 py-1.5 text-[11px] font-bold rounded-md transition-colors outline-none ${subVista === 'FIXTURE' ? 'bg-[#112a1f] text-white shadow' : 'text-[#6b7280] hover:text-[#112a1f]'}`}>TABLAS</button>
                  <button onClick={() => setSubVista('EQUIPOS')} className={`px-4 py-1.5 text-[11px] font-bold rounded-md transition-colors outline-none ${subVista === 'EQUIPOS' ? 'bg-[#112a1f] text-white shadow' : 'text-[#6b7280] hover:text-[#112a1f]'}`}>EQUIPOS</button>
                </div>
              </div>
              ...
              {subVista === 'FIXTURE' && (
                <>
                  {/* Select de Fechas */}
                  <div className="flex items-center justify-between bg-[#112a1f] rounded-lg px-3 py-2 mb-6 shadow-md border border-[#1f4a36]">
                    <button onClick={() => setFecha(prev => Math.max(1, prev - 1))} className="text-[#8cc63f] hover:text-white font-bold text-[16px] px-3 py-1 outline-none disabled:opacity-30 disabled:cursor-not-allowed" disabled={fecha === 1}>◀</button>
                    <select value={fecha} onChange={(e) => setFecha(Number(e.target.value))} className="w-full bg-transparent text-white font-bold text-[14px] px-2 outline-none text-center cursor-pointer uppercase tracking-wider">
                      {[...Array(temporada === '2023' ? 40 : (temporada === '2013' ? 48 : (temporada === '2018' ? 44 : 17)))].map((_, i) => {
                        let etiqueta = `FECHA ${i+1}`;
                        if (temporada === '2023') {
                            if (i < 19) {
                              const nombres = {1:'J. 3', 2:'J. 4', 3:'J. 5', 4:'J. 6', 5:'J. 7', 6:'J. 8', 7:'J. 9', 8:'J. 1 (Aplazada)', 9:'J. 10', 10:'J. 11', 11:'J. 12', 12:'J. 13', 13:'J. 14', 14:'J. 15', 15:'J. 2 (Aplazada)', 16:'J. 16', 17:'J. 17', 18:'J. 18', 19:'J. 19'};
                              etiqueta = `Semana ${i+1} (${nombres[i+1]})`;
                            } else if (i < 38) { etiqueta = `CLAUSURA F${i-18}`; } 
                            else if (i === 38) { etiqueta = `FINAL (IDA)`; } else if (i === 39) { etiqueta = `FINAL (VUELTA)`; }
                        }
                        return <option key={i+1} value={i+1} className="bg-white text-black font-semibold">{etiqueta}</option>
                      })}
                    </select>
                    <button onClick={() => setFecha(prev => Math.min(temporada === '2023' ? 40 : (temporada === '2013' ? 48 : (temporada === '2018' ? 44 : 17)), prev + 1))} className="text-[#8cc63f] hover:text-white font-bold text-[16px] px-3 py-1 outline-none disabled:opacity-30 disabled:cursor-not-allowed">▶</button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-[64%_34%] gap-6 items-start">
                    ...
                    <div className="flex flex-col gap-5">
                      ...
                      {temporada === '2013' && fecha > 30 && fecha <= 44 ? (<><TablaComponent titulo="LIGUILLA A" zona="(Puntos Acumulados)" datos={generarTabla(partidosValidos, liguillaA_2013, true)} esAcumulado={true} /><TablaComponent titulo="LIGUILLA B" zona="(Puntos Acumulados)" datos={generarTabla(partidosValidos, liguillaB_2013, true)} esAcumulado={true} /><TablaComponent titulo="TABLA GENERAL ACUMULADA" datos={generarTabla(partidosValidos, null, true)} esAcumulado={true} /></>) : (temporada === '2013' && <TablaComponent titulo={fecha > 44 ? "TABLA FINAL ACUMULADA" : `TABLA ACUMULADA (HASTA LA FECHA ${fecha})`} datos={generarTabla(temporada === '2013' && fecha > 44 ? listaPartidos.filter(p => p.Fecha_Global <= 44) : partidosValidos, null, true)} esAcumulado={true} />)}
                    </div>
                                                                                    
                    {/* COLUMNA RESULTADOS DE LA FECHA */}
                    <div className="flex flex-col gap-5">
                      <div className="bg-white border border-[#d1e0d7] rounded-lg overflow-hidden shadow-lg">
                        <div className="text-center font-bold text-[13px] uppercase py-[12px] bg-[#112a1f] text-white border-b border-[#1f4a36]">RESULTADOS DE LA FECHA</div>
                        <div className="flex flex-col max-h-[600px] overflow-y-auto custom-scrollbar">
                          {listaPartidos.filter(p => p.Fecha_Global === fecha).length === 0 ? (
                            <div className="text-center text-[12px] p-[20px] text-gray-500 font-medium">No hay partidos registrados.</div>
                          ) : (
                            listaPartidos.filter(p => p.Fecha_Global === fecha).map((p, idx) => {
                              ...
                              return (
                                <div key={idx} className={`flex justify-between items-center py-3 px-3 border-b border-[#d1e0d7] hover:bg-[#f8fbf9] ${idx % 2 === 0 ? 'bg-transparent' : 'bg-[#fcfdfc]'}`}>
                                  <div className="flex items-center w-full justify-center">
                                    <span className={`text-right w-[40%] text-[11px] font-bold truncate ${teamGanador === p.Local ? 'underline decoration-2 underline-offset-2 text-[#8cc63f]' : 'text-[#112a1f]'}`}>{p.Local}</span>
                                    ...
                                    <div className="flex items-center justify-center gap-0.5 mx-1 min-w-[55px]">
                                      {esWO ? ( <div className="text-[#d32f2f] text-[8px] font-black w-full text-center leading-tight">WALK<br/>OVER</div> ) : esConc ? ( <div className="text-[#d32f2f] text-[8px] font-black w-full text-center leading-tight">CONCE<br/>DIDO</div> ) : p.GL !== null && p.GV !== null ? ( <><div className="bg-[#112a1f] text-white rounded-[4px] font-bold text-[12px] w-[22px] h-[22px] flex items-center justify-center shadow-inner">{p.GL}</div><div className="font-black text-[12px] text-[#8cc63f] mx-0.5">-</div><div className="bg-[#112a1f] text-white rounded-[4px] font-bold text-[12px] w-[22px] h-[22px] flex items-center justify-center shadow-inner">{p.GV}</div></> ) : ( <div className="font-bold text-[10px] text-[#8cc63f] bg-[#112a1f] px-2 py-0.5 rounded">VS</div> )}
                                    </div>
                                    
                                    <img src={logos[p.Visitante]} className="w-[18px] h-[18px] mx-1.5 object-contain" />
                                    <span className={`text-left w-[40%] text-[11px] font-bold truncate ${teamGanador === p.Visitante ? 'underline decoration-2 underline-offset-2 text-[#8cc63f]' : 'text-[#112a1f]'}`}>{p.Visitante}</span>
                                  </div>
                                </div>
                              );
                            })
              ...
              {subVista === 'EQUIPOS' && (
                <div>
                  {!equipoSeleccionado ? (
                    <div className="animate-fade-in">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {Object.keys(logos).sort().filter(eq => {
                            if (temporada === '2026') return partidos2026Base.some(p => normalizarEquipoListado(p[1]) === eq || normalizarEquipoListado(p[2]) === eq);
                            ...
                            <button key={eq} onClick={() => setEquipoSeleccionado(eq)} className="p-5 flex flex-col items-center justify-center bg-white border border-[#d1e0d7] rounded-xl hover:bg-[#f0f4f2] hover:border-[#8cc63f] transition-all cursor-pointer group shadow-sm hover:shadow-md">
                    ...
                    <div className="animate-fade-in">
                      <button onClick={() => setEquipoSeleccionado(null)} className="mb-6 bg-[#112a1f] text-[#8cc63f] text-[11px] font-bold px-4 py-2 rounded-md hover:bg-[#1f4a36] transition-colors flex items-center gap-2 shadow">
                        ◀ VOLVER AL LISTADO
                      </button>
                      ...
                      {/* Oculté el componente interno temporalmente para no sobrepasar el código base, pero la estructura está limpia */}
                      <div className="text-center p-10 border-2 border-dashed border-[#d1e0d7] rounded-xl text-gray-500 font-bold">
                        ESTADÍSTICAS DETALLADAS EN CONSTRUCCIÓN 🚧
                      </div>
                    </div>
                  )}
                </div>
