...
# 1. CONFIGURACIÓN DE PÁGINA (WIDE y Tema Oscuro)
st.set_page_config(page_title="Liga Peruana 2018", page_icon="🏆", layout="wide")

# 2. INYECCIÓN DE CSS ACTUALIZADO (Nuevas clases para alineación perfecta de marcadores)
st.markdown("""
<style>
    /* Fondo principal verde oscuro de referencia y márgenes */
    ...
    /* TABLA HTML COMPACTA (Estilo Promiedos/Liga Arg.) */
    ...
    /* NUEVAS CLASES PARA ALINEACIÓN PERFECTA DEL MARCADOR (Lado Derecho) */
    .marcador-contenedor {
        display: flex;
        align-items: center;
    ...
    .gol-cajita {
        background-color: #082b1c; /* Fondo oscuro del cuadrito */
        border: 1px solid #1a4a2e; /* Borde sutil */
        border-radius: 4px;
        color: #ffffff;
        font-weight: bold;
        font-size: 14px;
        width: 25px; /* ANCHO FIJO: CLAVE PARA LA ALINEACIÓN */
        height: 25px;
        display: flex;
        align-items: center;
        justify-content: center;
    ...
    .separador-guion {
        color: #8cc63f; /* Color de acento verde claro para el guion */
        font-weight: bold;
        font-size: 14px;
    }
</style>
...
# --- AQUI PEGAS TUS LINKS MANUALMENTE (Guardados en memoria de la V14) ---
logos_equipos = {
...
# Volvimos a las Pestañas arriba para ordenar la vista compacta
...
with tab_fixture:
    ...
    with col_der:
        ...
        # 2. PANEL DE PARTIDOS REPARADO Y COMPACTO (Alineación perfecta)
        ...
        if not partidos_fecha.empty:
            ...
            for _, row in partidos_fecha.iterrows():
                ...
                # HTML TOTALMENTE COMPRIMIDO, USANDO CAJITAS PARA LOS GOLES
            ...
            st.markdown("<p style='text-align:center; font-size:11px;'>No hay partidos registrados en esta fecha.</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

        # 3. PANEL DE GOLEADORES (Ahora compacto a la derecha)
        ...
        if not df_goles_filtro.empty:
            ...
            # HTML comprimido, letras más chicas, logos de 14px
            ...
            st.markdown("<p style='text-align:center; font-size:11px;'>Aún no hay goles registrados hasta esta fecha.</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)


with col_izq:
    # 4. TABLA ACUMULADA PRINCIPAL (COMPACTA Y REPARADA)
    ...
    # Iniciamos la tabla sin scroll, estilo Liga Arg.
    ...
    for idx, row in df_t.iterrows():
        ...
        if pos <= 4: borde_color = "#3db4dc" # Libertadores
        elif pos <= 8: borde_color = "#e1c340" # Sudamericana
        elif pos >= 15: borde_color = "#d32f2f" # Descenso
        
        # HTML COMPRIMIDO TOTALMENTE PARA QUE NO SALGA COMO CÓDIGO (Sin saltos de línea)
